...
static gdouble matrix_vector_multiply_test(sisl_matrix_t *A, 
					   sisl_vector_t *b,
					   sisl_vector_t *w,
					   sisl_vector_t *r,
					   sisl_norm_t n)

{
  ...
  rc = (sisl_is_real(A) ? SISL_REAL : SISL_COMPLEX) ;
  
  fprintf(stderr, "unweighted %s (%s matrix)x(vector) multiplication\n",
	  sisl_complex_string(rc),
	  sisl_density_string(sisl_matrix_density(A))) ;
...

{
  ...
  fprintf(stderr, "unweighted %s (%s)x(%s) multiplication\n",
	  sisl_complex_string(rc),
	  sisl_density_string(sisl_matrix_density(A)),
	  sisl_density_string(sisl_matrix_density(B))) ;
...

{
  ...
  fprintf(stderr, "inversion of %s (%dx%d) %s matrix\n",
	  sisl_complex_string(rc), nr, nr,
	  sisl_density_string(sisl_matrix_density(A))) ;
...
static void matrix_matrix_mul_weighted_general_test(gint nr, gint nc,
...

{
  ...
  gint i, j, nra, nca, nrb, ncb ;
  
  fprintf(stderr, "weighted %s (%s)x(%s) multiplication\n",
	  sisl_complex_string(rc),
...
{
  matrix_matrix_mul_weighted_general_test(nr, nc,
					  SISL_MATRIX_DENSE,
					  SISL_MATRIX_DENSE,
					  rc) ;
  matrix_matrix_mul_weighted_general_test(nr, nc,
...

{
  ...
  sisl_logging_init(stderr, pstr, G_LOG_LEVEL_MESSAGE, wmpi_shutdown) ;
  ...
  if ( test == 0 ) {  
    ...
    res = matrix_vector_multiply_test(A, v, w, r, SISL_NORM_1) ;
